...
2. Read `.pstdio/docs/docs.json` to understand the current sidebar structure and available pages.
3. Apply the requested documentation changes:
   - **Adding a new page**: create the markdown file under `.pstdio/docs/`, then add a sidebar entry in `docs.json` with `{ "text": "<title>", "link": "<relative-path>" }`.
...
- Sidebar config: `.pstdio/docs/docs.json`
