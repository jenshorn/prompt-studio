...
2. Derive a concise, verb-led ticket title from the user request and immediately run the pstdio CLI to create the ticket.
   - Run `npx pstdio tickets write --title "<ticket title>" --input "<user prompt verbatim>" --status "<status>"`.
3. Fill the ticket template with concrete details:
   ...
   - If blocked, run `npx pstdio tickets update --id "<ticket-id>" --status blocked --blocked-reason <reason>`
...
6. Run `npx pstdio tickets save --id "<ticket id>"` to persist the updated ticket content.
