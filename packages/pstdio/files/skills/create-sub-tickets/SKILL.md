...
2. Load the parent ticket locally:
   - Run `npx pstdio tickets pull --id "<parent-ticket-id>"`.
...
5. Create each sub-ticket via the CLI (one command per sub-ticket):
   - Run `npx pstdio tickets write --title "<ticket title>" --status "<status>" --parent-id "<parent-ticket-id>"`.
...
9. Resolve blockers by checking all existing tickets that are not done `npx pstdio tickets list open`. If another ticket is a blocker, add it to `depends_on` in frontmatter. If blocked, run `npx pstdio tickets update --id “<ticket-id>” --status blocked --blocked-reason <reason>`
10. Run `npx pstdio tickets save --id “<ticket-id>”` for each child ticket to persist updates.
